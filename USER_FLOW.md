...
├─ 납품 목록
│  ├─ 각 항목: 고객사 + 금액 + 납품상태(청구상태)
│  └─ 클릭 → 상세 정보 표시, 카드에 다음 단게 버튼으로 빠르게 상태변경.
└─ FAB
   └─ "납품 등록" 버튼
```
...
AI 자동 분석 (고객명/금액/일정 추출)
    ↓
일정 자동 생성 (예정된 미팅 등록 + 고객사 자동 등록(비슷한 고객사명 고객사 테이블에서 확인 후 사용자에게 한 번 더 확인-혹시 이거 맞냐 아니냐로, 아니라면 고객사 추가, 맞다면 해당 고객사 자동 입력))
    ↓
