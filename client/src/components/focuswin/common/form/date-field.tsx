...
export default function DateField({ label, hint, error, required, className, value, onChange, inputProps }: DateFieldProps) {
  return (
    <div className={className}>
      <UiField label={typeof label === "string" && required ? `${label} *` : (label as any)} hint={error ?? hint}>
        <Input type="date" value={value} onChange={e => onChange(e.target.value)} aria-invalid={!!error} {...inputProps} />
      </UiField>
    </div>
  );
}
