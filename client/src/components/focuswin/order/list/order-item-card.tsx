  ...
  return (
    <WorkItemCard interactive>
      ...
      <div className="min-w-0 flex-1">
        ...
        {/* 품목 */}
        <p className="mt-0.5 text-sm text-slate-500 truncate">{order.prod_serv}</p>

        {/* 금액 */}
        <div className="mt-1 text-lg font-black text-slate-900">{formatKRW(Number(order.orde_pric))}</div>

        <WorkItemCard.Body>
          {isCanceled ? (
            /* 취소된 수주는 step 없이 납기일 메타만 */
            order.expd_date ? (
              <p className="text-xs text-slate-400">납기: {new Date(order.expd_date).toLocaleDateString("ko-KR")}</p>
            ) : null
          ) : (
            <StepProgress.Combo
              steps={ORDER_STEPS}
              ...
              leftMeta={
                <div className="flex items-center gap-3">
                  {order.expd_date ? (
                    <p className="text-xs font-semibold text-slate-500">납기: {new Date(order.expd_date).toLocaleDateString("ko-KR")}</p>
                  ) : (
