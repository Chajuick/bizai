...
const navItems = [
  ...
  { path: "/ship-list", label: "납품", icon: TrendingUp, shortLabel: "납품" },
  { path: "/expe-list", label: "지출", icon: Receipt, shortLabel: "지출" },
...
function WorkspaceSwitcher() {
  const { currentCompIdno, currentCompName, planCode, companies, switchCompany } = useWorkspaceSwitcher();
  const [open, setOpen] = useState(false);
  const [, navigate] = useLocation();

  return (
    <div className="relative">
      <button className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-2xl hover:bg-slate-50 transition text-left" onClick={() => setOpen(!open)} aria-label="워크스페이스 전환">
        <Building2 size={15} className="text-slate-400 shrink-0" />
        <span className="flex-1 text-sm font-semibold text-slate-800 truncate min-w-0">{currentCompName ?? "로딩 중…"}</span>
        {planCode && <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${PLAN_BADGE_CLASS[planCode] ?? PLAN_BADGE_CLASS.free}`}>{planCode}</span>}
              ...
              companies.map(c => (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-4 py-3 text-sm hover:bg-slate-50 transition text-left ${
                    c.comp_idno === currentCompIdno ? "text-blue-700 font-semibold" : "text-slate-700"
                  }`}
                  onClick={() => {
                ...
                >
                  ...
                  {c.comp_idno === currentCompIdno && <Check size={13} className="text-blue-600 shrink-0" />}
                </button>
              ))}
...
function AccountPopover({ userInitial, userName, userEmail, onLogout }: { userInitial: string; userName: string; userEmail: string; onLogout: () => void }) {
  const [open, setOpen] = useState(false);
  const [, navigate] = useLocation();
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="w-full flex items-center gap-3 px-2 py-2 rounded-2xl hover:bg-slate-50 transition cursor-pointer group" aria-label="계정 메뉴 열기">
          <div
          ...
          <ChevronRight size={14} className="text-slate-400 group-hover:text-slate-600 transition shrink-0" />
        </button>
      </PopoverTrigger>
    ...
    <div className="h-[100dvh] overflow-hidden flex bg-[color:var(--blueprint-bg)]">
      ...
      >
        ...
        <div className="p-4 border-t" style={{ borderColor: "var(--blueprint-border)" }}>
          <AccountPopover userInitial={userInitial} userName={userName} userEmail={userEmail} onLogout={logout} />
        </div>
      </aside>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          >
            ...
            <div className="p-4 border-t pb-6" style={{ borderColor: "var(--blueprint-border)" }}>
              <button
              ...
              <Button variant="outline" size="sm" className="w-full mt-3 text-xs rounded-2xl text-red-500 border-red-100 hover:bg-red-50" onClick={() => logout()}>
