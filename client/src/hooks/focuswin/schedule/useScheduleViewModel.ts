...
export function usePromisesViewModel() {
  ...
  const list = useMemo((): EnhancedPromise[] => {
    ...
    const rows = (schedulesData?.items ?? []) as PromiseRow[];

    return rows.map((p) => {
      const scheduledMs = new Date(p.sche_date).getTime();
      const overdue = p.stat_code === "scheduled" && scheduledMs < kstTodayMidnightMs;
      const imminent =
        p.stat_code === "scheduled" &&
        !overdue &&
        scheduledMs > nowMs &&
  ...
  const displayList = useMemo((): EnhancedPromise[] => {
    ...
    if (activeTab === "scheduled") return list.filter(p => p.stat_code === "scheduled" && !p.overdue && !p.imminent);
    if (activeTab === "completed" || activeTab === "canceled") return list.filter(p => p.stat_code === activeTab);
  ...
  const counts = useMemo(() => {
    ...
    for (const p of list) {
      ...
      else base[p.stat_code as DbPromiseStatus]++;
    }
    return base;
  }, [list]);
